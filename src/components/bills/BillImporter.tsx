    ...
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        const dateIndex = header.findIndex(
          (h) => h.includes("date") || h.includes("data")
        );
        ...
        const typeIndex = header.findIndex(
          (h) => h.includes("type") || h.includes("tipo")
        );
        const currencyIndex = header.findIndex(
          (h) => h.includes("currency") || h.includes("moeda")
        );
        ...
        const description =
          values[descriptionIndex] || `Imported transaction ${i}`;
        const type =
          values[typeIndex]?.toLowerCase().includes("income") ||
          values[typeIndex]?.toLowerCase().includes("receita")
            ? "income"
            : "expense";
